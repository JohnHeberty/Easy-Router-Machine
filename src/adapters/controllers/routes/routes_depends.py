...
from src.infra.db.repositories.routes_repository import UploadDfLocsRepository
...
def get_upload_locs_df_repository() -> InsertLocRepository:
    '''Cria e retorna o Repositório de Upload de locs com o worker já instanciado.'''
    task_service = create_task_service()
    return UploadDfLocsRepository(task_service.get_worker())
...
def get_upload_locs_df_user_case() -> InsertLocUserCase:
    '''Instancia e retorna o caso de uso para fazer upload locais.'''
    return UploadDfLocsUserCase(get_upload_locs_df_repository())